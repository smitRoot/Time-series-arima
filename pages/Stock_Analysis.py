...
from pages.utils.plotly_figure import MACD,plotly_table,filter_data,close_chart,candlesticks,RSI,Moving_average
import datetime
import ta
...
print(data.head())
data.columns = data.columns.get_level_values(0)  # Keep only the first level (Price types)
print(data.head())

daily_chnage=data['Close'].iloc[-1] - data['Close'].iloc[-2]
col1.metric("Daily change", str(round(data['Close'].iloc[-1],2)),str(round(daily_chnage,2)))

last_10_df=data.tail(10).sort_index(ascending=False).round(3)

fig_df=plotly_table(last_10_df)
print(last_10_df.head())
...
if num_period== '':
    if chart_type=='Candle' and indicators == 'RSI':
        st.plotly_chart(candlesticks(data1,'1y'),use_container_width=True)
    ...
    if chart_type=='Candle' and indicators == 'RSI':
        st.plotly_chart(candlesticks(data1,num_period),use_container_width=True)
        st.plotly_chart(RSI(data1,'1y'),use_container_width=True)
    ...
    if chart_type=='Line' and indicators == 'MACD':
        st.plotly_chart(close_chart(data1,),use_container_width=True)
        st.plotly_chart(MACD(data1,'1y'),use_container_width=True)
